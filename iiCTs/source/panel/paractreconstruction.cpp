...
ParaCTReconstruction::ParaCTReconstruction(QWidget* parent)
    : QWidget(parent)
{
    ...
    valueParams = {
        { "PixelCountX", -1, "" },       { "LayerThickness", -1, "mm" },
        { "PixelCountY", -1, ""},        { "XOffset", -1, "mm" },
        { "PixelSizeX", -1, "mm" },      { "YOffset", -1, "mm" },
        { "PixelSizeY", -1, "mm" },      { "ZOffset", -1, "mm" },
    };

    fileParams  = {
        {"ReconAlgorithm", "-1"}, {"ReconKernel", "-1"},
        {"DetectorGeometry", "/home/"},
        {"CTGeometry", "/home/"},
        { "InputPath", "/home/" },
        { "OutputPath", "/home/" }
    };

    const QStringList fileSelectionKeys = { "DetectorGeometry", "CTGeometry"};
    const QStringList floderSelectionKeys = { "OutputPath", "InputPath"};
    const QStringList materialKeys = { "AnodeMaterial", "CathodeMaterial", "FocusMaterial"};


    userFileDir = allInfo.ProjectPath + "/" + allInfo.ProjectName + "/" + allInfo.ReconFolder;
    systemFileDir = allInfo.SystemScriptPath + "/recon";
...
//    UIControlLayout->addStretch();
    ...
    connect(this, &ParaCTReconstruction::fileSaved,
...
void ParaCTReconstruction::resizeEvent(QResizeEvent* event)
{
    QWidget::resizeEvent(event);

}

void ParaCTReconstruction::setMainWindow(MainWindow* main) {
    mainWindow = main;
}

void ParaCTReconstruction::paintEvent(QPaintEvent *event)
{
...
void ParaCTReconstruction::showStyledWarning(QWidget *parent, const QString &titleText, const QString &messageText) {
...
void ParaCTReconstruction::onProjectInfoChanged(const ProjectInfo &info)
{
    allInfo = info;
    userFileDir = allInfo.ProjectPath + "/" + allInfo.ProjectName + "/" + allInfo.ReconFolder;
    systemFileDir = allInfo.SystemScriptPath + "/recon";
...
void ParaCTReconstruction::populateUICombo()
{
    fileUICombo->clear();
...
void ParaCTReconstruction::populateCombo()
{
    fileCombo->clear();
    auto scanDir = [&](const QString& dirPath, const QString& tag){
...
void ParaCTReconstruction::populateTargetCombo()
{
    TargetCombo->clear();
    auto scanDir = [&](const QString& dirPath, const QString& tag) {
        QDirIterator it(dirPath, QDir::Files, QDirIterator::Subdirectories);
...
void ParaCTReconstruction::PlaneRunFunction(const QString &targetPath,
