...
ParaTubeSimulation::ParaTubeSimulation(QWidget* parent)
    : QWidget(parent)
{
    ...
    valueParams = {
        { "AnodeTiltAngle", -1, "°" },    { "CathodePotential", -1, "KeV" },
        { "AnodeRadius", -1, "mm" },      { "AnodePotential", -1, "KeV" },
        { "AnodeThickness", -1, "mm" },   { "FocusPotential", -1, "KeV" },
        { "FocusRadius", -1, "mm" },      { "CathodeRadius", -1, "mm" },
        { "FocusThickness", -1, "mm"},    { "CathodeToAnode", -1, "mm" },
        { "FocusHoleRadius", -1, "mm" },  { "CathodeThickness", -1, "mm" },
        { "CathodeLength", -1, "mm" },    { "GateRadius", -1, "mm" },
        //        { "CathodeTurns", -1, "" },       { "GateThickness", -1, "mm" },
        //        { "FocusDepth", -1, "mm" },       { "GateHoleRadius", -1, "mm" },
        //        { "FilamentTemp", -1, "mm" },     { "SubstrateRadius", -1, "mm" },
    };

    fileParams  = {
        {"OutputFilename", "-1"},         {"AnodeMaterial", "-1"},
        {"CathodeMaterial", "-1"},        {"FocusMaterial", "-1"},
        { "OutputPath", "/home/" }
    };

    const QStringList fileSelectionKeys = { "xxxxx", "yyyyyy" };
    const QStringList floderSelectionKeys = { "OutputPath"};
    const QStringList materialKeys = { "AnodeMaterial", "CathodeMaterial", "FocusMaterial"};


    userFileDir = allInfo.ProjectPath + "/" + allInfo.ProjectName + "/" + allInfo.TubeFolder;
    systemFileDir = allInfo.SystemScriptPath + "/tube/" + allInfo.CTType;
    ...
    connect(this, &ParaTubeSimulation::fileSaved,
...
void ParaTubeSimulation::resizeEvent(QResizeEvent* event)
{
    QWidget::resizeEvent(event);

}

void ParaTubeSimulation::setMainWindow(MainWindow* main) {
    mainWindow = main;
}

void ParaTubeSimulation::paintEvent(QPaintEvent *event)
{
...
void ParaTubeSimulation::showStyledWarning(QWidget *parent, const QString &titleText, const QString &messageText) {
...
void ParaTubeSimulation::onProjectInfoChanged(const ProjectInfo &info)
{
    allInfo = info;
    userFileDir = allInfo.ProjectPath + "/" + allInfo.ProjectName + "/" + allInfo.TubeFolder;
    systemFileDir = allInfo.SystemScriptPath + "/tube/" + allInfo.TubeType;
...
void ParaTubeSimulation::populateUICombo()
{
    fileUICombo->clear();
...
void ParaTubeSimulation::populateCombo()
{
    fileCombo->clear();
    auto scanDir = [&](const QString& dirPath, const QString& tag){
...
void ParaTubeSimulation::populateTargetCombo()
{
    TargetCombo->clear();
    auto scanDir = [&](const QString& dirPath, const QString& tag) {
        QDirIterator it(dirPath, QDir::Files, QDirIterator::Subdirectories);
...
void ParaTubeSimulation::PlaneRunFunction(const QString &targetPath,
...
//    qDebug() << "Simulating...";
    ...
    showStyledWarning(this, "Error", "Please use the commercial CST Studio Suite software to perform the simulation.");
}
