...
#include "transhelicalct.h"
#include "transcbct.h"
#include "translinearct.h"
...
ParaCTScanSimulation::ParaCTScanSimulation(QWidget *parent)
    : QWidget(parent)
{
    ...
    const QStringList fileSelectionKeys = {"DetectorPath"};
    const QStringList floderSelectionKeys = {"SpectrumPath", "PhantomPath", "OutputPath"};
    const QStringList materialKeys = {"AnodeMaterial", "CathodeMaterial", "FocusMaterial"};

    userFileDir = allInfo.ProjectPath + "/" + allInfo.ProjectName + "/" + allInfo.CTFolder;
    systemFileDir = allInfo.SystemScriptPath + "/ct";
    ...
    connect(TargetCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
        ...
        if (idx >= 0) {
            ...
            // 注意：不在这里发出 fileResultChanged 信号
            // 因为选择GWL文件不是"结果改变"，只是选择了输入文件
            // fileResultChanged 应该在转换完成后或有实际结果时发出
             emit fileResultChanged(TargetPath);  // 已移除：避免Result面板尝试解析GWL文件
    ...
    connect(this, &ParaCTScanSimulation::fileSaved,
    ...
    // 初始化GWL转换器
    helicalConverter = new TransHelicalCT(this);
    cbctConverter = new TransCBCT(this);
    linearConverter = new TransLinearCT(this);
    polygonIntervalConverter = new PolygonIntervalCT(this);
    polygonSideConverter = new TransPolygonSideCT(this);

    connect(helicalConverter, &TransHelicalCT::conversionCompleted,
            this, [this](const QString &message)
            {
                qDebug() << "Helical CT GWL Conversion completed:" << message;
                if (mainWindow) {
                    TerminalWidget *term = mainWindow->terminal();
                    term->appendLine("<font color='green'>Helical CT conversion completed: " + message + "</font>");
                } });
    connect(helicalConverter, &TransHelicalCT::conversionError,
            this, [this](const QString &error)
            {
                qDebug() << "Helical CT GWL Conversion error:" << error;
                if (mainWindow) {
                    TerminalWidget *term = mainWindow->terminal();
                    term->appendLine("<font color='red'>Helical CT conversion error: " + error + "</font>");
                } });

    connect(cbctConverter, &TransCBCT::conversionCompleted,
            this, [this](const QString &message)
            {
                qDebug() << "CBCT GWL Conversion completed:" << message;
                if (mainWindow) {
                    TerminalWidget *term = mainWindow->terminal();
                    term->appendLine("<font color='green'>CBCT conversion completed: " + message + "</font>");
                } });
    connect(cbctConverter, &TransCBCT::conversionError,
            this, [this](const QString &error)
            {
                qDebug() << "CBCT GWL Conversion error:" << error;
                if (mainWindow) {
                    TerminalWidget *term = mainWindow->terminal();
                    term->appendLine("<font color='red'>CBCT conversion error: " + error + "</font>");
                } });

    connect(linearConverter, &TransLinearCT::conversionCompleted,
            this, [this](const QString &message)
            {
                qDebug() << "Linear CT GWL Conversion completed:" << message;
                if (mainWindow) {
                    TerminalWidget *term = mainWindow->terminal();
                    term->appendLine("<font color='green'>Linear CT conversion completed: " + message + "</font>");
                } });
    connect(linearConverter, &TransLinearCT::conversionError,
            this, [this](const QString &error)
            {
                qDebug() << "Linear CT GWL Conversion error:" << error;
                if (mainWindow) {
                    TerminalWidget *term = mainWindow->terminal();
                    term->appendLine("<font color='red'>Linear CT conversion error: " + error + "</font>");
                } });

    connect(polygonIntervalConverter, &PolygonIntervalCT::conversionCompleted,
            this, [this](const QString &message)
            {
                qDebug() << "Polygon Interval CT GWL Conversion completed:" << message;
                if (mainWindow) {
                    TerminalWidget *term = mainWindow->terminal();
                    term->appendLine("<font color='green'>Polygon Interval CT conversion completed: " + message + "</font>");
                } });
    connect(polygonIntervalConverter, &PolygonIntervalCT::conversionError,
            this, [this](const QString &error)
            {
                qDebug() << "Polygon Interval CT GWL Conversion error:" << error;
                if (mainWindow) {
                    TerminalWidget *term = mainWindow->terminal();
                    term->appendLine("<font color='red'>Polygon Interval CT conversion error: " + error + "</font>");
                } });

    connect(polygonSideConverter, &TransPolygonSideCT::conversionCompleted,
            this, [this](const QString &message)
            {
                qDebug() << "Polygon Side CT GWL Conversion completed:" << message;
                if (mainWindow) {
                    TerminalWidget *term = mainWindow->terminal();
                    term->appendLine("<font color='green'>Polygon Side CT conversion completed: " + message + "</font>");
                } });
    connect(polygonSideConverter, &TransPolygonSideCT::conversionError,
            this, [this](const QString &error)
            {
                qDebug() << "Polygon Side CT GWL Conversion error:" << error;
                if (mainWindow) {
                    TerminalWidget *term = mainWindow->terminal();
                    term->appendLine("<font color='red'>Polygon Side CT conversion error: " + error + "</font>");
                } });
}

void ParaCTScanSimulation::resizeEvent(QResizeEvent *event)
{
    QWidget::resizeEvent(event);
}

void ParaCTScanSimulation::setMainWindow(MainWindow *main)
{
    mainWindow = main;
}

void ParaCTScanSimulation::paintEvent(QPaintEvent *event)
{
...
void ParaCTScanSimulation::showStyledWarning(QWidget *parent, const QString &titleText, const QString &messageText)
{
...
void ParaCTScanSimulation::onProjectInfoChanged(const ProjectInfo &info)
{
    allInfo = info;
    userFileDir = allInfo.ProjectPath + "/" + allInfo.ProjectName + "/" + allInfo.CTFolder;
    systemFileDir = allInfo.SystemScriptPath + "/ct";
...
void ParaCTScanSimulation::populateUICombo()
{
    fileUICombo->clear();
...
void ParaCTScanSimulation::populateCombo()
{
    fileCombo->clear();
    auto scanDir = [&](const QString &dirPath, const QString &tag)
    {
...
void ParaCTScanSimulation::populateTargetCombo()
{
    TargetCombo->clear();
    auto scanDir = [&](const QString &dirPath, const QString &tag)
    {
...
void ParaCTScanSimulation::PlaneRunFunction(const QString &targetPath,
                                            const ProjectInfo &info)
{
    ...
    term->appendLine("Start CT simulation...");
    ...
    QString macFileName = baseName + "_loop.mac";
    ...
    connect(geant4Process, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
            [=](int exitCode, QProcess::ExitStatus exitStatus)
            {
                if (exitStatus == QProcess::NormalExit)
                {
                    if (exitCode == 0)
                    {
                        term->appendLine("<font color='green'>CT simulation completed successfully！</font>");
                    }
                    else
                    {
                        term->appendLine("<font color='orange'>CT simulation finished, exit code: " + QString::number(exitCode) + "</font>");
                    }
                }
                else
                {
    ...
    {
        term->appendLine("<font color='red'>Failed to launch the Geant4 CT project! Please verify that the paths and commands are correct.</font>");
        ...
        term->appendLine("- CT Type: " + info.CTType);
        geant4Process->deleteLater();
        return;
    }

    term->appendLine("<font color='green'>Geant4 CT project launched; simulation is now running...</font>");
}

QString ParaCTScanSimulation::detectGeant4ProjectPath(const ProjectInfo &info, TerminalWidget *term)
{
    ...
    if (projectDir.cdUp())
    {
        ...
        if (geant4ObjectDir.exists())
        {
            ...
            QDir ctProjectDir(ctProjectPath);
            if (ctProjectDir.exists())
            {
                QString buildPath = ctProjectPath + "/build";
                QDir buildDir(buildPath);
                if (buildDir.exists())
                {
                ...
                else
                {
                    ...
                    return ctProjectPath; // 返回项目路径，即使build目录不存在
                }
            }
            else
            {
                term->appendLine("<font color='red'>Error: " + ctProjectFolder + " directory not found in: " + geant4ObjectPath + "</font>");
                term->appendLine("<font color='yellow'>Available folders in Geant4Object:</font>");
                QStringList folders = geant4ObjectDir.entryList(QDir::Dirs | QDir::NoDotAndDotDot);
                for (const QString &folder : folders)
                {
                    term->appendLine("  - " + folder);
                }
            }
        }
        else
...
QString ParaCTScanSimulation::detectGeant4InstallPath(TerminalWidget *term)
{
    // 常见的Geant4安装路径
...
QString ParaCTScanSimulation::getCTProjectFolder(const QString &ctType)
{
    // 根据CTType映射到相应的Geant4项目文件夹
    if (ctType == "helical")
    {
        return "HelicalCT";
    }
    ...
    else if (ctType == "linear_static")
    {
        return "LinearCT";
    }
    else if (ctType == "polygon_interleaved")
    {
        return "StaticCT_interval";
    }
    else if (ctType == "polygon_side")
    {
...
void ParaCTScanSimulation::convertSelectedGWLFile()
{
    if (!mainWindow)
        return;
...
void ParaCTScanSimulation::convertSelectedGWLFileWithIndex(int idx)
{
    if (!mainWindow)
        return;
    ...
    term->appendLine("Debug: allInfo.CTType = '" + allInfo.CTType + "'");
    ...
    term->appendLine("Debug: fileName = '" + fileName + "', baseName = '" + baseName + "', extension = '" + extension + "'");

    if (extension != "gwl")
    {
        term->appendLine("Debug: Skipping non-GWL file: " + fileName);
        return;
    }
    ...
    if (geant4ProjectPath.isEmpty())
    {
        term->appendLine("<font color='red'>Failed to locate Geant4 CT project for conversion!</font>");
        return;
    }

    QString buildPath = geant4ProjectPath + "/build";

    // 5. 执行GWL转Mac转换 - 根据CTType选择合适的转换器
    term->appendLine("Auto-converting GWL to Mac file: " + baseName);
    term->appendLine("Selected template file: " + fileName);
    term->appendLine("GWL source file: " + fullPath);
    ...
    if (allInfo.CTType == "helical")
    {
        term->appendLine("Using TransHelicalCT converter...");
        term->appendLine("Debug: helicalConverter pointer = " + QString::number(reinterpret_cast<quintptr>(helicalConverter), 16));

        // 检查GWL文件是否可读
        QFile gwlFile(fullPath);
        if (!gwlFile.open(QIODevice::ReadOnly | QIODevice::Text))
        {
            term->appendLine("<font color='red'>Error: Cannot open GWL file for reading: " + fullPath + "</font>");
            return;
        }

        // 读取并显示GWL文件的前几行用于调试
        QTextStream gwlStream(&gwlFile);
        term->appendLine("Debug: GWL file content (first 10 lines):");
        int lineCount = 0;
        while (!gwlStream.atEnd() && lineCount < 10)
        {
            QString line = gwlStream.readLine();
            term->appendLine("  " + QString::number(lineCount + 1) + ": " + line);
            lineCount++;
        }

        gwlFile.close();
        term->appendLine("Debug: GWL file is readable");

        // 检查目标目录是否存在且可写
        QDir buildDir(buildPath);
        if (!buildDir.exists())
        {
            term->appendLine("<font color='orange'>Warning: Build directory does not exist, creating: " + buildPath + "</font>");
            if (!buildDir.mkpath(buildPath))
            {
                term->appendLine("<font color='red'>Error: Cannot create build directory: " + buildPath + "</font>");
                return;
            }
        }
        term->appendLine("Debug: Build directory exists and is accessible");

        if (helicalConverter)
        {
            conversionSuccess = helicalConverter->convertGWLToMac(fullPath, macFilePath);
            term->appendLine("Debug: helicalConverter->convertGWLToMac returned: " + QString::number(conversionSuccess));
        }
        else
        {
            term->appendLine("<font color='red'>Error: helicalConverter is null!</font>");
            return;
        }

        if (!conversionSuccess)
        {
            term->appendLine("<font color='red'>Error: Failed to convert GWL to Mac file using HelicalCT converter!</font>");
            return;
        }
    }
    else if (allInfo.CTType == "CBCT")
    {
        term->appendLine("Using TransCBCT converter...");
        term->appendLine("Debug: cbctConverter pointer = " + QString::number(reinterpret_cast<quintptr>(cbctConverter), 16));
        ...
        if (cbctConverter)
        {
            conversionSuccess = cbctConverter->convertGWLToMac(fullPath, macFilePath);
            term->appendLine("Debug: cbctConverter->convertGWLToMac returned: " + QString::number(conversionSuccess));
        }
        else
        {
            term->appendLine("<font color='red'>Error: cbctConverter is null!</font>");
            return;
        }

        if (!conversionSuccess)
        {
            term->appendLine("<font color='red'>Error: Failed to convert GWL to Mac file using CBCT converter!</font>");
            return;
        }
    }
    else if (allInfo.CTType == "linear_static")
    {
        term->appendLine("Using TransLinearCT converter...");
        term->appendLine("Debug: linearConverter pointer = " + QString::number(reinterpret_cast<quintptr>(linearConverter), 16));
        ...
        if (linearConverter)
        {
            conversionSuccess = linearConverter->convertGWLToMac(fullPath, macFilePath);
            term->appendLine("Debug: linearConverter->convertGWLToMac returned: " + QString::number(conversionSuccess));
        }
        else
        {
            term->appendLine("<font color='red'>Error: linearConverter is null!</font>");
            return;
        }

        if (!conversionSuccess)
        {
            term->appendLine("<font color='red'>Error: Failed to convert GWL to Mac file using Linear CT converter!</font>");
            return;
        }
    }
    else if (allInfo.CTType == "polygon_interleaved")
    {
        term->appendLine("Using PolygonIntervalCT converter...");
        term->appendLine("Debug: polygonIntervalConverter pointer = " + QString::number(reinterpret_cast<quintptr>(polygonIntervalConverter), 16));
        ...
        if (polygonIntervalConverter)
        {
            conversionSuccess = polygonIntervalConverter->convertGWLToMac(fullPath, macFilePath);
            term->appendLine("Debug: polygonIntervalConverter->convertGWLToMac returned: " + QString::number(conversionSuccess));
        }
        else
        {
            term->appendLine("<font color='red'>Error: polygonIntervalConverter is null!</font>");
            return;
        }

        if (!conversionSuccess)
        {
            term->appendLine("<font color='red'>Error: Failed to convert GWL to Mac file using Polygon Interval CT converter!</font>");
            return;
        }
    }
    else if (allInfo.CTType == "polygon_side")
    {
        term->appendLine("Using TransPolygonSideCT converter...");
        term->appendLine("Debug: polygonSideConverter pointer = " + QString::number(reinterpret_cast<quintptr>(polygonSideConverter), 16));
        ...
        if (polygonSideConverter)
        {
            conversionSuccess = polygonSideConverter->convertGWLToMac(fullPath, macFilePath);
            term->appendLine("Debug: polygonSideConverter->convertGWLToMac returned: " + QString::number(conversionSuccess));
        }
        else
        {
            term->appendLine("<font color='red'>Error: polygonSideConverter is null!</font>");
            return;
        }

        if (!conversionSuccess)
        {
            term->appendLine("<font color='red'>Error: Failed to convert GWL to Mac file using Polygon Side CT converter!</font>");
            return;
        }
    }
    else
    {
        term->appendLine("<font color='orange'>Warning: CT Type '" + allInfo.CTType + "' not supported for GWL conversion yet.</font>");
        return;
    }

    // 检查转换是否真的成功生成了文件
    QFile generatedMacFile(macFilePath);
    if (generatedMacFile.exists())
    {
        qint64 fileSize = generatedMacFile.size();
        term->appendLine("<font color='green'>GWL conversion completed: " + macFileName + "</font>");
        term->appendLine("<font color='green'>Generated Mac file size: " + QString::number(fileSize) + " bytes</font>");
        ...
        {
            qint64 visFileSize = visMacFile.size();
            term->appendLine("<font color='green'>Generated Vis Mac file: " + baseName + "_vis.mac (" + QString::number(visFileSize) + " bytes)</font>");
        }
        else
        {
            term->appendLine("<font color='orange'>Warning: Visualization Mac file not generated: " + visMacFilePath + "</font>");
        }
        // --- 新增: 同时复制 mac/vis/loop 到用户项目的 MACFolder 下，按类型分类 ---
        // 构造用户项目的 mac 目录路径
        ...
        typeMap["CBCT"] = "CBCT";
        typeMap["helical"] = "HelicalCT";
        typeMap["linear_static"] = "LinearCT";
        typeMap["spectrum"] = "spectrum";
        typeMap["polygon_interleaved"] = "StaticCT_interval";
        typeMap["polygon_side"] = "StaticCT_Side";

        QString targetSubFolder = typeMap.value(allInfo.CTType, "StaticCT_Side");
        ...
        // 如果 loop 文件存在，也复制（仅针对 helical 可能生成）
        QString loopFilePath = buildPath + "/" + baseName + "_loop.mac";
        QFile loopFile(loopFilePath);
        if (loopFile.exists())
        {
            QString destLoopPath = destDirPath + "/" + baseName + "_loop.mac";
            if (QFile::exists(destLoopPath))
                QFile::remove(destLoopPath);
            if (QFile::copy(loopFilePath, destLoopPath))
            {
                term->appendLine("<font color='green'>Copied Loop Mac to project folder: " + destLoopPath + "</font>");
            }
            else
            {
                term->appendLine("<font color='orange'>Warning: Failed to copy Loop Mac to project folder: " + destLoopPath + "</font>");
            }
        }
    }
    else
    {
        term->appendLine("<font color='red'>Error: Mac file was not generated despite conversion success: " + macFilePath + "</font>");
    }
}

QString ParaCTScanSimulation::extractTemplateNameFromComboText(const QString &comboText)
{
    // 处理格式如: "• template_name (user)" 或 "• template_name (system)"
