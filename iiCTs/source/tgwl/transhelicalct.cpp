...
TransHelicalCT::TransHelicalCT(QObject *parent)
    : QObject(parent)
{
}

TransHelicalCT::~TransHelicalCT()
{
}

bool TransHelicalCT::convertGWLToMac(const QString &gwlFilePath, const QString &macFilePath)
{
    ...
    // 生成循环控制文件（如果有 TotalViews、CircleViews 和 StartIndex 参数）
...
bool TransHelicalCT::parseGWLFile(QFile *gwlFile, HelicalCTGWLData &gwlData)
{
...
bool TransHelicalCT::parseParameter(const QString &paramName, const QString &paramValue, HelicalCTGWLData &gwlData)
{
    // 几何参数
...
bool TransHelicalCT::generateMacFile(const QString &macFilePath, const HelicalCTGWLData &gwlData)
{
    ...
    // 探测器角度参数：优先使用 DetectorAngle 字段，否则当 DetYaw/DetPitch/DetRoll 全部存在时组合输出
...
bool TransHelicalCT::generateVisMacFile(const QString &visFilePath, const HelicalCTGWLData &gwlData)
{
...
QString TransHelicalCT::generateVisMacFilePath(const QString &macFilePath)
{
    QFileInfo fileInfo(macFilePath);
...
bool TransHelicalCT::generateLoopControlFile(const QString &loopFilePath, const HelicalCTGWLData &gwlData)
{
