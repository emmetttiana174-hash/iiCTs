...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
    ...
    if (fPolygonCount == 3){
        if (fOrder <= fTotalsource/3.0){
            ...
            G4ThreeVector centre(sourceXPos+fUOffset-3*(fOrder-1)*fdetLength/(fTotalsource-3)+fdetLength/2,sourceYPos+fVOffset, -fdetLength/(2*std::tan(30*deg)));
            ...
            G4RotationMatrix rot;
            rot.rotateX(fTubeXRotationAngle); // 绕 X 轴旋转
            rot.rotateY(fTubeYRotationAngle+120 * CLHEP::pi / 180.0); // 绕 Y 轴旋转
            rot.rotateZ(fTubeZRotationAngle); // 绕 Z 轴旋转
            direction = rot * direction;
            G4ThreeVector centre((sourceXPos+fUOffset-3*(2.0*fTotalsource/3.0-fOrder)*fdetLength/(fTotalsource-3))*std::cos(60*deg)-fdetLength/2, sourceYPos+fVOffset, (sourceXPos+fUOffset+3*(fOrder-fTotalsource/3.0-1)*fdetLength/(fTotalsource-3))*std::sin(60*deg));
            ...
            rot.rotateZ(fTubeZRotationAngle); // 绕 Z 轴旋转
            direction = rot * direction;
            G4ThreeVector centre((sourceXPos+fUOffset+3*(fOrder-2.0*fTotalsource/3.0-1)*fdetLength/(fTotalsource-3))*std::cos(60*deg)+fdetLength/2, sourceYPos+fVOffset, (sourceXPos+fUOffset+3*(fTotalsource-fOrder)*fdetLength/(fTotalsource-3))*std::sin(60*deg));
