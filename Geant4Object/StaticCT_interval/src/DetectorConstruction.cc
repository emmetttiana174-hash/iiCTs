...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
    ...
    fPhysDetector = new G4PVPlacement(fRotationMatrix2, detPos1, fLogicDetector, "Detector", logicWorld, false, 0, false);
    ...
    for (size_t i = 0; i < fPhantomDatas.size(); ++i) {
        ...
        if (fBedDirection == 1){
            ...
            fPhysObject=new G4PVPlacement(rot, phantomPos, logicPhantom, "Phantom" + std::to_string(i), logicWorld, false, 0);
        }else{
            G4ThreeVector phantomPos((data.minX + data.maxX) / 2  * mm +fObjectXBias,
                        -(phantomZ * mm+fdegree*fDetectorWidth*fPitch/360 +fObjectYBias),
                        (data.minY + data.maxY) / 2  * mm+ fObjectZBias);
    ...
    if (fPolygonCount == 2){
        //DetectorFilter
        G4Box* Filter = new G4Box("Filter", fFilterLength/2.0, fFilterWidth/2.0, fFilterHeight/2.0);
        fLogicFilter = new G4LogicalVolume(Filter, fFilterMaterial, "Filter");
        G4ThreeVector detPos2( fFilterXBias, fFilterYBias,  -fFilterPos+fDetectorLength/2-fFilterHeight/2.0 );
        G4RotationMatrix* rotation = new G4RotationMatrix();
        rotation->rotateX(0 * CLHEP::pi / 180.0);
        fPhysFilter = new G4PVPlacement(rotation, detPos2, fLogicFilter, "Filter", logicWorld, false, 0, false);
        
        fLogicFilter1 = new G4LogicalVolume(Filter, fFilterMaterial, "Filter");
        G4ThreeVector detPos3( fFilterXBias-fFilterPos+fDetectorLength/2-fFilterHeight/2.0, fFilterYBias,0 );
        G4RotationMatrix* rotation1 = new G4RotationMatrix();
        rotation1->rotateY(-90 * CLHEP::pi / 180.0);
        fPhysFilter = new G4PVPlacement(rotation1, detPos3, fLogicFilter, "Filter", logicWorld, false, 1, false);

        //TubeFilter
        G4Box* TubeFilter = new G4Box("TubeFilter", fTubeFilterLength/2.0, fTubeFilterWidth/2.0, fTubeFilterHeight/2.0);
        fLogicTubeFilter = new G4LogicalVolume(TubeFilter, fTubeFilterMaterial, "TubeFilter");
        G4ThreeVector detPos4( fTubeFilterXBias, fTubeFilterYBias,  -fDetectorLength/2+fTubeFilterPos+fTubeFilterHeight/2.0 );
        fPhysTubeFilter = new G4PVPlacement(0, detPos4, fLogicTubeFilter, "TubeFilter", logicWorld, false, 0, false);

        fLogicTubeFilter1 = new G4LogicalVolume(TubeFilter, fTubeFilterMaterial, "TubeFilter");
        G4ThreeVector detPos5( -fDetectorLength/2+fTubeFilterPos+fTubeFilterHeight/2.0 ,fTubeFilterXBias, fTubeFilterYBias);
        G4RotationMatrix* rotation2 = new G4RotationMatrix();
        rotation2->rotateY(-90 * CLHEP::pi / 180.0);
        fPhysTubeFilter = new G4PVPlacement(rotation2, detPos5, fLogicTubeFilter, "TubeFilter", logicWorld, false, 0, false);
    }else if (fPolygonCount == 3){
        //DetectorFilter
        G4Box* Filter = new G4Box("Filter", fFilterLength/2.0, fFilterWidth/2.0, fFilterHeight/2.0);
        fLogicFilter = new G4LogicalVolume(Filter, fFilterMaterial, "Filter");
        G4ThreeVector detPos( fDetectorBiasLength, fDetectorBiasWidth, fDetectorLength/(2*std::tan(30*deg)) - fFilterHeight/2-fFilterPos);
        //G4ThreeVector detPos2( fFilterXBias, fFilterYBias,  -fFilterPos+fDetectorLength/2-fFilterHeight/2.0 );
        G4RotationMatrix* rotation = new G4RotationMatrix();
        rotation->rotateX(0 * CLHEP::pi / 180.0);
        fPhysFilter = new G4PVPlacement(rotation, detPos, fLogicFilter, "Filter", logicWorld, false, 0, false);
        
        fLogicFilter1 = new G4LogicalVolume(Filter, fFilterMaterial, "Filter");
        G4ThreeVector detPos1( (fDetectorLength/(2*std::tan(30*deg))- fFilterHeight/2-fFilterPos)*std::cos(30*deg), fDetectorBiasWidth, (fDetectorLength/(2*std::tan(30*deg))- fFilterHeight/2-fFilterPos)*std::sin(30*deg));
        //G4ThreeVector detPos3( fFilterXBias-fFilterPos+fDetectorLength/2-fFilterHeight/2.0, fFilterYBias,0 );
        G4RotationMatrix* rotation1 = new G4RotationMatrix();
        rotation1->rotateY(120 * CLHEP::pi / 180.0);
        fPhysFilter = new G4PVPlacement(rotation1, detPos1, fLogicFilter, "Filter", logicWorld, false, 1, false);

        fLogicFilter2 = new G4LogicalVolume(Filter, fFilterMaterial, "Filter");
        G4ThreeVector detPos2( -(fDetectorLength/(2*std::tan(30*deg))- fFilterHeight/2-fFilterPos)*std::cos(30*deg), fDetectorBiasWidth, (fDetectorLength/(2*std::tan(30*deg))- fFilterHeight/2-fFilterPos)*std::sin(30*deg));
        //G4ThreeVector detPos3( fFilterXBias-fFilterPos+fDetectorLength/2-fFilterHeight/2.0, fFilterYBias,0 );
        G4RotationMatrix* rotation2 = new G4RotationMatrix();
        rotation2->rotateY(60 * CLHEP::pi / 180.0);
        fPhysFilter = new G4PVPlacement(rotation2, detPos2, fLogicFilter, "Filter", logicWorld, false, 1, false);

        //TubeFilter
        G4Box* TubeFilter = new G4Box("TubeFilter", fTubeFilterLength/2.0, fTubeFilterWidth/2.0, fTubeFilterHeight/2.0);
        fLogicTubeFilter = new G4LogicalVolume(TubeFilter, fTubeFilterMaterial, "TubeFilter");
        G4ThreeVector detPos3( fDetectorBiasLength, fDetectorBiasWidth, -fDetectorLength/(2*std::tan(30*deg)) + fFilterHeight/2 + fTubeFilterPos);
        fPhysTubeFilter = new G4PVPlacement(0, detPos3, fLogicTubeFilter, "TubeFilter", logicWorld, false, 0, false);

        fLogicTubeFilter1 = new G4LogicalVolume(TubeFilter, fTubeFilterMaterial, "TubeFilter");
        G4ThreeVector detPos4( (fDetectorLength/(2*std::tan(30*deg))- fTubeFilterHeight/2-fTubeFilterPos)*std::cos(30*deg), fDetectorBiasWidth, -(fDetectorLength/(2*std::tan(30*deg))- fTubeFilterHeight/2-fTubeFilterPos)*std::sin(30*deg));
        G4RotationMatrix* rotation3 = new G4RotationMatrix();
        rotation3->rotateY(60 * CLHEP::pi / 180.0);
        fPhysTubeFilter = new G4PVPlacement(rotation3, detPos4, fLogicTubeFilter, "TubeFilter", logicWorld, false, 0, false);

        fLogicTubeFilter2 = new G4LogicalVolume(TubeFilter, fTubeFilterMaterial, "TubeFilter");
        G4ThreeVector detPos5( -(fDetectorLength/(2*std::tan(30*deg))- fTubeFilterHeight/2-fTubeFilterPos)*std::cos(30*deg), fDetectorBiasWidth, -(fDetectorLength/(2*std::tan(30*deg))- fTubeFilterHeight/2-fTubeFilterPos)*std::sin(30*deg));
        G4RotationMatrix* rotation4 = new G4RotationMatrix();
        rotation4->rotateY(120 * CLHEP::pi / 180.0);
        fPhysTubeFilter = new G4PVPlacement(rotation4, detPos5, fLogicTubeFilter, "TubeFilter", logicWorld, false, 0, false);
    }
    if (fPolygonCount == 2){
        // 定义探测器，使用动态尺寸
        G4Box* solidDetector = new G4Box("Detector", fDetectorLength/2, fDetectorWidth/2, fDetectorHeight/2); // 5 cm × 5 cm × 1 cm
        fLogicDetector = new G4LogicalVolume(solidDetector, fWorldMaterial, "Detector"); // 设置为类成员
        G4ThreeVector detPos( fDetectorBiasLength, fDetectorBiasWidth, fDetectorLength/2+ fDetectorHeight/2);
        fPhysDetector = new G4PVPlacement(fRotationMatrix3, detPos, fLogicDetector, "Detector", logicWorld, false, 0, false);

        fLogicDetector1 = new G4LogicalVolume(solidDetector, fWorldMaterial, "Detector"); // 设置为类成员
        G4ThreeVector detPos1( fDetectorLength/2+fDetectorHeight/2, fDetectorBiasWidth, fDetectorBiasLength);
        fPhysDetector1 = new G4PVPlacement(fRotationMatrix4, detPos1, fLogicDetector1, "Detector", logicWorld, false, 1, false);

        G4Box* cellSolid = new G4Box("cell", fPixelLength/2, fPixelWidth/2, fPixelHeight/2); // 0.01 cm × 0.01 cm × 1 cm
        cellLogical = new G4LogicalVolume(cellSolid, fDetectorMaterial, "cell"); // 使用相同9材料

        G4double x = fDetectorLength/fPixelLength,y = fDetectorWidth/fPixelWidth;
        for (G4int i = 0; i < x; ++i) {
        for (G4int j = 0; j < y; ++j){
            //计算单元中心坐标（相对于母体框架）
            G4double xPos = (-fDetectorLength/2 + fPixelLength/2) + i * fPixelLength; // 沿 X 轴均匀分布
            G4double yPos = (-fDetectorWidth/2 + fPixelWidth/2) + j * fPixelWidth;
            G4double zPos = 0.;                                     // Z 轴居中
        ...
        for (G4int j = 0; j < y; ++j){
            ...
            G4double yPos = (-fDetectorWidth/2 + fPixelWidth/2) + j * fPixelWidth;
            G4double zPos = 0.;                                     // Z 轴居中
            new G4PVPlacement(0, G4ThreeVector(xPos, yPos, zPos), cellLogical, "cell", fLogicDetector1,
                            false, i*y + j + x*y, false);
        }
        }
    }else if (fPolygonCount == 3){
        // 定义探测器，使用动态尺寸
        G4Box* solidDetector = new G4Box("Detector", fDetectorLength/2, fDetectorWidth/2, fDetectorHeight/2); // 5 cm × 5 cm × 1 cm
        fLogicDetector = new G4LogicalVolume(solidDetector, fWorldMaterial, "Detector"); // 设置为类成员
        G4ThreeVector detPos( fDetectorBiasLength, fDetectorBiasWidth, fDetectorLength/(2*std::tan(30*deg))+ fDetectorHeight/2);
        fPhysDetector = new G4PVPlacement(fRotationMatrix6, detPos, fLogicDetector, "Detector", logicWorld, false, 0, false);

        fLogicDetector1 = new G4LogicalVolume(solidDetector, fWorldMaterial, "Detector"); // 设置为类成员
        G4ThreeVector detPos1( (fDetectorLength/(2*std::tan(30*deg))+ fDetectorHeight/2)*std::cos(30*deg), fDetectorBiasWidth, (fDetectorLength/(2*std::tan(30*deg))+ fDetectorHeight/2)*std::sin(30*deg));
        fPhysDetector1 = new G4PVPlacement(fRotationMatrix7, detPos1, fLogicDetector1, "Detector", logicWorld, false, 1, false);

        fLogicDetector2 = new G4LogicalVolume(solidDetector, fWorldMaterial, "Detector"); // 设置为类成员
        G4ThreeVector detPos2( -(fDetectorLength/(2*std::tan(30*deg))+ fDetectorHeight/2)*std::cos(30*deg), fDetectorBiasWidth, (fDetectorLength/(2*std::tan(30*deg))+ fDetectorHeight/2)*std::sin(30*deg));
        fPhysDetector2 = new G4PVPlacement(fRotationMatrix5, detPos2, fLogicDetector2, "Detector", logicWorld, false, 2, false);

        G4cout << "fPixelLength" << fPixelLength << "fPixelWidth" << fPixelWidth << "fPixelHeight" << fPixelHeight << G4endl;
        G4Box* cellSolid = new G4Box("cell", fPixelLength/2, fPixelWidth/2, fPixelHeight/2); // 0.01 cm × 0.01 cm × 1 cm
        cellLogical = new G4LogicalVolume(cellSolid, fDetectorMaterial, "cell"); // 使用相同9材料
        ...
        for (G4int j = 0; j < y; ++j){
            ...
            G4double yPos = (-fDetectorWidth/2 + fPixelWidth/2) + j * fPixelWidth;
            G4double zPos = 0.;                                     // Z 轴居中
            new G4PVPlacement(0, G4ThreeVector(xPos, yPos, zPos), cellLogical, "cell", fLogicDetector2,
                            false, i*y + j + 2*x*y, false);
        }
        }
    }
  return fPhysWorld;
...
void DetectorConstruction::UpdateGeometry()
{
    ...
    G4RunManager::GetRunManager()->ReinitializeGeometry(true); // 强制重建几何
...
void DetectorConstruction::SetPolygonCount(G4double PolygonCount) {
    fPolygonCount = PolygonCount;
    G4cout << "fPolygonCount" << fPolygonCount << G4endl;
...
void DetectorConstruction::SetDetectorRotationAngle1(G4double angleX, G4double angleY, G4double angleZ) {
    if (fPhysDetector) {
        *fRotationMatrix3 = G4RotationMatrix();
        fRotationMatrix3->rotateX(angleX);
        fRotationMatrix3->rotateY(angleY);
        fRotationMatrix3->rotateZ(angleZ);
        fPhysDetector->SetRotation(fRotationMatrix3);
        G4cout << "RotationMatrix3: " << *fRotationMatrix3 << G4endl;
        G4cout << "fPhysDetector: " << (fPhysDetector ? "exists" : "null") << G4endl;
        G4RunManager::GetRunManager()->GeometryHasBeenModified();
        if (fPhysWorld) {
        G4RunManager::GetRunManager()->ReinitializeGeometry();
        }
...
void DetectorConstruction::SetDetectorRotationAngle2(G4double angleX, G4double angleY, G4double angleZ) {
    if (fPhysDetector) {
        *fRotationMatrix4 = G4RotationMatrix();
        G4cout << "angleX" << angleX << " angleY" << angleY << " angleZ" << angleZ << G4endl;
        fRotationMatrix4->rotateX(angleX);
        fRotationMatrix4->rotateY(angleY+90 * CLHEP::pi / 180.0);
        fRotationMatrix4->rotateZ(angleZ);
        fPhysDetector->SetRotation(fRotationMatrix4);
        G4cout << "RotationMatrix3: " << *fRotationMatrix4 << G4endl;
        G4cout << "fPhysDetector1: " << (fPhysDetector1 ? "exists" : "null") << G4endl;
        G4RunManager::GetRunManager()->GeometryHasBeenModified();
        if (fPhysWorld) {
        G4RunManager::GetRunManager()->ReinitializeGeometry();
        }
...
void DetectorConstruction::SetDetectorRotationAngle3(G4double angleX, G4double angleY, G4double angleZ) {
    if (fPhysDetector) {
        *fRotationMatrix5 = G4RotationMatrix();
        fRotationMatrix5->rotateX(angleX);
        fRotationMatrix5->rotateY(angleY+60 * CLHEP::pi / 180.0);
        fRotationMatrix5->rotateZ(angleZ);
        fPhysDetector->SetRotation(fRotationMatrix5);
        G4RunManager::GetRunManager()->GeometryHasBeenModified();
        if (fPhysWorld) {
        G4RunManager::GetRunManager()->ReinitializeGeometry();
        }
...
void DetectorConstruction::SetDetectorRotationAngle4(G4double angleX, G4double angleY, G4double angleZ) {
    if (fPhysDetector) {
        *fRotationMatrix6 = G4RotationMatrix();
        fRotationMatrix6->rotateX(angleX);
        fRotationMatrix6->rotateY(angleY);
        fRotationMatrix6->rotateZ(angleZ);
        fPhysDetector->SetRotation(fRotationMatrix6);
        G4RunManager::GetRunManager()->GeometryHasBeenModified();
        if (fPhysWorld) {
        G4RunManager::GetRunManager()->ReinitializeGeometry();
        }
...
void DetectorConstruction::SetDetectorRotationAngle5(G4double angleX, G4double angleY, G4double angleZ) {
    if (fPhysDetector) {
        *fRotationMatrix7 = G4RotationMatrix();
        fRotationMatrix7->rotateX(angleX);
        fRotationMatrix7->rotateY(angleY+120 * CLHEP::pi / 180.0);
        fRotationMatrix7->rotateZ(angleZ);
        fPhysDetector->SetRotation(fRotationMatrix7);
        G4RunManager::GetRunManager()->GeometryHasBeenModified();
        if (fPhysWorld) {
        G4RunManager::GetRunManager()->ReinitializeGeometry();
        }
