...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
    ...
    totaltheta = 360.0*fSourceLength/(CLHEP::pi * (fDSO + fOffcenter)) * CLHEP::deg;
    if (fOffcenter != 0){
        G4cout << "forder" << fOrder << "ftotalsource" << fTotalsource << G4endl;
        if (fTotalsource % 2 == 0){
            if (fOrder < (fTotalsource/2) ){
                midangle = totaltheta/2 - (fOrder-1)*totaltheta/(fTotalsource-1);
                fz = -std::cos(midangle) * (fDSO + fOffcenter);
                fx = std::sin(midangle) * (fDSO + fOffcenter);
                // 计算方向向量
                G4ThreeVector direction(x, y, z);
                direction = direction.unit();
                G4RotationMatrix rot;
                rot.rotateX(fTubeXRotationAngle);
                rot.rotateY(fTubeYRotationAngle-midangle);
                rot.rotateZ(fTubeZRotationAngle);
                fDirection = rot * direction;
            }else if (fOrder == (fTotalsource/2)){
                midangle = totaltheta/(2*(fTotalsource-1));
                fz = -std::cos(midangle) * (fDSO + fOffcenter);
                fx = std::sin(midangle) * (fDSO + fOffcenter);
                // 计算方向向量
                G4ThreeVector direction(x, y, z);
                ...
                rot.rotateZ(fTubeZRotationAngle);
                fDirection = rot * direction;
            }else if (fOrder == (fTotalsource/2) + 1){
                midangle = totaltheta/(2*(fTotalsource-1));
                fz = -std::cos(midangle) * (fDSO + fOffcenter);
                fx = -std::sin(midangle) * (fDSO + fOffcenter);
                // 计算方向向量
                ...
                midangle = totaltheta/2 - (fTotalsource-fOrder)*totaltheta/(fTotalsource-1);
                fz = -std::cos(midangle) * (fDSO + fOffcenter);
                fx = -std::sin(midangle) * (fDSO + fOffcenter);
                // 计算方向向量
                G4ThreeVector direction(x, y, z);
            ...
            G4ThreeVector centre(fx+sourceXPos+fUOffset,sourceYPos+fVOffset, fz+fOffcenter);
            ...
            primaryParticle->SetMomentumDirection(fDirection);
            vertex->SetPrimary(primaryParticle);
            anEvent->AddPrimaryVertex(vertex);
        }else{
            if (fOrder < (fTotalsource/2) ){
                ...
                G4cout << "fz: " << fz << G4endl;
                G4cout << "fx: " << fx << G4endl;
                G4cout << "midangle: " << midangle << G4endl;
                G4cout << "midangle*deg: " << midangle / CLHEP::deg << G4endl;
                G4cout << "totaltheta: " << totaltheta << G4endl;
                G4cout << "totaltheta*deg: " << totaltheta/ CLHEP::deg << G4endl;
                G4cout << "fTubeYRotationAngle: " << fTubeYRotationAngle << G4endl;
                // 计算方向向量
                G4ThreeVector direction(x, y, z);
                direction = direction.unit();  // 归一化
                G4RotationMatrix rot;
                rot.rotateX(fTubeXRotationAngle); // 绕 X 轴旋转
                rot.rotateY(fTubeYRotationAngle-midangle); // 绕 Y 轴旋转
        ...
        G4RotationMatrix rot;
        rot.rotateX(fTubeXRotationAngle);
        rot.rotateY(fTubeYRotationAngle);
        rot.rotateZ(fTubeZRotationAngle);
        fDirection = rot * direction;
        fx = fSourceLength/2 - (fOrder - 1) * fSourceLength / (fTotalsource - 1);
        G4ThreeVector centre(fx+sourceXPos+fUOffset,sourceYPos+fVOffset, -fDSO);
        //G4ThreeVector centre(sourceXPos+fUOffset,sourceYPos+fVOffset, -fDSO);
        G4ThreeVector position = centre;
        G4PrimaryVertex* vertex = new G4PrimaryVertex(position, 0);
        // 根据编号读取对应的能量文件
...
void PrimaryGeneratorAction::SetSourceLength(G4double SourceLength)
{
    fSourceLength = SourceLength;
}
void PrimaryGeneratorAction::SetOffcenter(G4double Offcenter)
{
    fOffcenter = Offcenter;
}