...
DetectorConstructionMessenger::DetectorConstructionMessenger(DetectorConstruction* detector)
    ...
    fDetectorAngleCmd = new G4UIcommand("/detector/setDetectorRotationAngle", this);
    fDetectorAngleCmd->SetGuidance("Set Detector rotation angles in radians (angleX angleY angleZ)");
    ...
    fDetectorAngleCmd->SetParameter(angleXParam);
    fDetectorAngleCmd->SetParameter(angleYParam);
    fDetectorAngleCmd->SetParameter(angleZParam);
    ...
    fDetectorXAngleCmd = new G4UIcommand("/detector/setDetectorXRotationAngle", this);
    fDetectorXAngleCmd->SetGuidance("Set Detector rotation angles in radians (angleX)");
    G4UIparameter* XAngle = new G4UIparameter("angle", 'd', false);  // Double type, not optional
    fDetectorXAngleCmd->SetParameter(XAngle);

    fDetectorYAngleCmd = new G4UIcommand("/detector/setDetectorYRotationAngle", this);
    fDetectorYAngleCmd->SetGuidance("Set Detector rotation angles in radians (angleY)");
    G4UIparameter* YAngle = new G4UIparameter("angle", 'd', false);  // Double type, not optional
    fDetectorYAngleCmd->SetParameter(YAngle);

    fDetectorZAngleCmd = new G4UIcommand("/detector/setDetectorZRotationAngle", this);
    fDetectorZAngleCmd->SetGuidance("Set Detector rotation angles in radians (angleZ)");
    G4UIparameter* ZAngle = new G4UIparameter("angle", 'd', false);  // Double type, not optional
    fDetectorZAngleCmd->SetParameter(ZAngle);
...
DetectorConstructionMessenger::~DetectorConstructionMessenger() {
    ...
    delete fDetectorXAngleCmd;
    delete fDetectorYAngleCmd;
    delete fDetectorZAngleCmd;
...
void DetectorConstructionMessenger::SetNewValue(G4UIcommand* cmd, G4String newValue) {
    ...
    }else if (cmd == fDetectorAngleCmd) {
        std::istringstream iss(newValue);
        G4double angleX, angleY, angleZ;
        iss >> angleX >> angleY >> angleZ;
        fDetector->SetDetectorRotationAngle(angleX* deg, angleY* deg, angleZ* deg);
    }else if (cmd == fMultipleCmd) {
        std::istringstream iss(newValue);
        G4double multiple;
        iss >> multiple;
        fDetector->SetMultiple(multiple);
    }else if (cmd == fDetectorXAngleCmd) {
        std::istringstream iss(newValue);
        G4double angleX;
        iss >> angleX ;
        fDetector->SetDetectorXRotationAngle(angleX * deg);
    }else if (cmd == fDetectorYAngleCmd) {
        std::istringstream iss(newValue);
        G4double angleY;
        iss >> angleY ;
        fDetector->SetDetectorYRotationAngle(angleY * deg);
    }else if (cmd == fDetectorZAngleCmd) {
        std::istringstream iss(newValue);
        G4double angleZ;
        iss >> angleZ ;
        fDetector->SetDetectorZRotationAngle(angleZ * deg);
