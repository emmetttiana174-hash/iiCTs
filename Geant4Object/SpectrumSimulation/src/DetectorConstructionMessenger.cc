    ...
    : fDetector(detector)
{
    ...
    fFilter1PosCmd->SetGuidance("Set filter1 Z position (absolute)\nExample: /Filter/Filter1Position 1 mm sets position to (0,0,1mm)");
    ...
    fFilter2PosCmd->SetGuidance("Set filter2 Z position (absolute)\nExample: /Filter/Filter2Position 2 mm sets position to (0,0,2mm)");
...
void DetectorConstructionMessenger::SetNewValue(G4UIcommand *cmd, G4String newValue)
{
    ...
    else if (cmd == fFilter1PosCmd)
    {
        ...
        G4cout << "Messenger: received /Filter/Filter1Position " << posZ / mm << " mm" << G4endl;
        fDetector->SetFilter1Position(posZ);
    }
    ...
    {
        G4double posZ = G4UIcmdWithADoubleAndUnit::GetNewDoubleValue(newValue);
        G4cout << "Messenger: received /Filter/Filter2Position " << posZ / mm << " mm" << G4endl;
        fDetector->SetFilter2Position(posZ);
    }
