...
G4VPhysicalVolume *DetectorConstruction::Construct()
{
    ...
    // 2. 创建椭球体 (直径17mm压缩到厚度5mm)
    G4double radius_xy = 17 * mm; // xy平面半径 (直径17mm的一半)
    G4double radius_z = 2.5 * mm; // z方向半径 (厚度5mm的一半)

    G4Ellipsoid *ellipsoid = new G4Ellipsoid("AnodeEllipsoid",
                                             radius_xy, // x半轴
                                             radius_xy, // y半轴
                                             radius_z); // z半轴

    // 3. 创建用于切割的盒子
    G4double box_size = 20.0 * mm;
    G4Box *cutting_box = new G4Box("CuttingBox",
                                   box_size,
                                   box_size,
                                   box_size);
    ...
    rotation->rotateY(5.0 * deg); // 绕y轴旋转5度

    // 5. 设置切割盒子的位置
    // 盒子向z正方向偏移，确保能切掉椭球体的上半部分
    G4double offset_z = box_size * cos(5.0 * deg) - radius_z * sin(5.0 * deg);
    G4ThreeVector cutting_position(0, 0, offset_z);

    // 6. 执行布尔减法运算
    G4SubtractionSolid *anode_solid = new G4SubtractionSolid("AnodeSolid",
                                                             ellipsoid,
                                                             cutting_box,
                                                             rotation,
                                                             cutting_position);
    ...
    // 9. 放置阳极靶面在世界体中 (支持动态旋转)
    G4RotationMatrix *anodeRotation = new G4RotationMatrix();
    anodeRotation->rotateZ(-90.0 * deg);         // 基础Z轴旋转90度
    anodeRotation->rotateY(-fAnodeRotationAngle); // 加上用户设定的绕X轴的旋转角度

    fPhysAnode = new G4PVPlacement(anodeRotation,          // 旋转矩阵
                                   G4ThreeVector(0, 0, 0), // 位于原点
                                   fAnodeLogical,          // 逻辑体
                                   "AnodePhysical",        // 名称
                                   logicWorld,             // 母体积
                                   false,                  // 无布尔运算
    ...
    G4cout << "Created Anode Target at origin:" << G4endl;
    G4cout << "  - Material: " << fAnodeMaterialName << G4endl;
    G4cout << "  - Diameter: 17mm (compressed to 5mm thickness)" << G4endl;
    G4cout << "  - Cut at 5 degree angle in xy plane" << G4endl;
    ...
    detectorRotation->rotateX(-5.0 * deg);          // 基础X轴旋转5度
    detectorRotation->rotateX(fAnodeRotationAngle); // 与阳极靶同步绕X轴旋转

    // Place the detection plane at z = 5 cm
    G4double detectorPosX = 0 * cm;
    G4double detectorPosY = 0 * cm;
    G4double detectorPosZ = 0.6 * cm;
    ...
    filterRotation->rotateX(-5.0 * deg);          // 基础X轴旋转5度
    filterRotation->rotateX(fAnodeRotationAngle); // 与阳极靶同步绕X轴旋转
    ...
    filter2Rotation->rotateX(-5.0 * deg);          // 基础X轴旋转5度
    filter2Rotation->rotateX(fAnodeRotationAngle); // 与阳极靶同步绕X轴旋转

    fPhysFilter2 = new G4PVPlacement(
        filter2Rotation,                                      // 与阳极靶同步旋转
        G4ThreeVector(filter2PosX, filter2PosY, filter2PosZ), // Position at (0,0,0.4cm)
    ...
    G4cout << "滤过层1: 材料=" << fFilterMaterialName << ", 位置=(0,0,0.3cm)" << G4endl;
    G4cout << "滤过层2: 材料=" << fFilter2MaterialName << ", 位置=(0,0,0.4cm)" << G4endl;
...
void DetectorConstruction::SetAnodeRotationAngle(G4double angle)
{
    ...
    if (fPhysAnode && fPhysDetector && fPhysFilter && fPhysFilter2)
    {
        ...
        anodeRotation->rotateX(fAnodeRotationAngle); // 用户设定的绕X轴自转角度
        ...
        G4cout << "All components rotation angle updated to: " << fAnodeRotationAngle / deg << " degrees" << G4endl;
    }
}
