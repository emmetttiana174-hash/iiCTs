...
PhysicsList::PhysicsList(): G4VModularPhysicsList()
{
  ...
  // 初始化物理过程开关状态（默认全部启用）
  fEnableIonisation = true;
  fEnableBremsstrahlung = true;
  fEnableScatter = true;
  fEnableAnnihilation = true;
...
void PhysicsList::ApplyPhysicsProcessControls()
{
  // 获取粒子表
  ...
  while((*particleIterator)()) {
    ...
    if (particle->GetParticleName() == "e-" || particle->GetParticleName() == "e+") {
      ...
      // 控制电离过程
      if (!fEnableIonisation) {
        G4VProcess* ionProcess = pmanager->GetProcess("eIoni");
        if (ionProcess) {
          pmanager->SetProcessActivation(ionProcess, false);
          G4cout << "Disabled ionisation for " << particle->GetParticleName() << G4endl;
        }
      }
      
      // 控制韧致辐射过程
      if (!fEnableBremsstrahlung) {
        G4VProcess* bremProcess = pmanager->GetProcess("eBrem");
        if (bremProcess) {
          pmanager->SetProcessActivation(bremProcess, false);
          G4cout << "Disabled bremsstrahlung for " << particle->GetParticleName() << G4endl;
        }
      }
      
      // 控制多重散射过程
      if (!fEnableScatter) {
        G4VProcess* mscProcess = pmanager->GetProcess("msc");
        if (mscProcess) {
          pmanager->SetProcessActivation(mscProcess, false);
          G4cout << "Disabled multiple scattering for " << particle->GetParticleName() << G4endl;
        }
      }
    }
    
    // 控制正电子湮灭过程
    if (particle->GetParticleName() == "e+" && !fEnableAnnihilation) {
      G4VProcess* annihiProcess = pmanager->GetProcess("annihil");
      if (annihiProcess) {
        pmanager->SetProcessActivation(annihiProcess, false);
        G4cout << "Disabled annihilation for " << particle->GetParticleName() << G4endl;
      }
    }
  }
}
